...
module.exports = function(Promise, drive) {
  var QL = {
    ...
    qualify: function() {
      ...
      Object.keys(this.drivers).forEach(function(key) {
        ...
        if (self.fastest === undefined) {
          ...
          if (driver.fast < self.fastest.fast) {
            self.fastest = driver;
          }
        }
    ...
    _selectNoCache: function(obj) {
      ...
      console.dir(self.tables);
      console.dir(obj.params.table);
      console.dir(obj.source.tables);
    ...
    */
    _selectFromCache: function(obj) {
      ...
      if (obj.params.id === undefined) {
        if (this.cache[obj.params.table]["__status"] === "complete") {
          ...
          if (Object.keys(obj.params.order).length > 0) {
            // sort the output
            resolved.sort(function(a, b) {
              var score = 0;

              Object.keys(obj.params.order).every(function(column) {
                var direction = obj.params.order[column];
    ...
    selectFrom: function(table, id) {
      ...
      return {
        ...
        send: function() {
          console.log("Sending SELECT with parameters :");
